...
import type {
    WorkPermitSubmitRequest,
    WorkPermitSubmitResponse,
...
serve(async (req) => {
    ...
    try {
        const body: WorkPermitSubmitRequest = await req.json();
        ...
        const { data: appData, error: appError } = await supabase
            .from("applications")
            .select("id, org_id, processing_context, submitted_at")
            .eq("id", applicationId)
            .single();
        ...
        // =========================================================================
        // 2. Get work permit attributes and current status expiry
        // =========================================================================
        ...
        // Pre-check evaluation (before setting submittedAt)
        const preEvaluation = evaluateFromData({
            statusExpiryAt,
            submittedAt: null,
        ...
        if (preEvaluation.blockers.length > 0) {
            const response: WorkPermitSubmitResponse = {
                success: false,
        ...
        // =========================================================================
        const finalEvaluation = evaluateFromData({
            statusExpiryAt,
            submittedAt,
            processingContext: appData.processing_context,
            actionIntent: wpAttrs?.action_intent || null,
        ...
        console.error("wp-submit error:", error);
